...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='indian',
                        choices=['indian','pavia','houston','salina'],
                        help='dataset name')
    ## network setting
    parser.add_argument('--network', type=str, default='FContNet',
                        choices=['FContNet'],
                        help='network name')
    ...
    parser.add_argument('--use_apex', type=str, default='True',
    ...
    ## model setting
    parser.add_argument('--mode', type=str, default='pcs',
    ...
    for count in range(0, args.experiment_num):
        ...
        if args.network == 'FContNet':
            from models.FullyContNet import fucontnet
            net = fucontnet(args,X_data.shape[1],categories-1)
        else:
            raise NotImplementedError
        ...
        if cohen_kappa_score(y_tes_gt,y_tes)>=kappa or count==0:
            if args.resume:
                torch.save(net, str(args.network) + '_' + str(FLAG) + '_' + str(args.head) + '_' + str(
                    args.mode) + '_ft.pth')
                kappa = cohen_kappa_score(y_tes_gt, y_tes)
                np.save(str(args.network) + '_' + str(FLAG) + '_' + str(args.head) + '_' + str(args.mode) +
                        '_pre_map_ft.npy', y_tes_pred)
            else:
                torch.save(net,str(args.network)+'_'+str(FLAG)+'_'+str(args.head)+'_'+str(args.mode)+'.pth')
                kappa=cohen_kappa_score(y_tes_gt,y_tes)
                np.save(str(args.network)+'_'+str(FLAG)+'_'+str(args.head)+'_'+str(args.mode)+
                             '_pre_map.npy',y_tes_pred)
    ...
    if args.resume:
        scio.savemat(str(args.network) + '_' + str(FLAG) + '_' + str(args.head) + '_' + str(args.mode) +
                     '_ft.mat', {'data': Experiment_result})

        y_disp_all = np.load(str(args.network) + '_' + str(FLAG) + '_' + str(args.head) + '_' + str(args.mode) +
                             '_pre_map_ft.npy')

        cv2.imwrite(str(args.network) + '_' + str(FLAG) + '_' + str(args.head) + '_' + str(args.mode) +
                    '_segmap_ft.png', y_disp_all.reshape(r, c))
    else:
        scio.savemat(str(args.network)+'_'+str(FLAG)+'_'+str(args.head)+'_'+str(args.mode)+
                     '.mat',{'data':Experiment_result})

        y_disp_all = np.load(str(args.network)+'_'+str(FLAG)+'_'+str(args.head)+'_'+str(args.mode)+
                             '_pre_map.npy')

        cv2.imwrite(str(args.network)+'_'+str(FLAG) +'_'+str(args.head)+'_'+str(args.mode)+
                    '_segmap.png', y_disp_all.reshape(r,c))
